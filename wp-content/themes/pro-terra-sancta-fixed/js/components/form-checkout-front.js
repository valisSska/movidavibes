          ...
          <>
            ...
            >
              <span className={`text-menu ${singleDonation ? 'selected' : ''}`}>
                {locale[window.language].contributo}
              </span>
            </div>
          </>
        )}
      ...
      {formType !== 'in-memory' && (
        <div className="donate-selected-checkout row gx-0">
          <div className="col-8 text-container row align-items-center">
            <div>
              <div className="icon-title">{locale[window.language].checkout}</div>
          ...
          <div className="input-container selected">
            <div className="md-form">
              ...
              <input
                ...
                placeholder={locale[window.language].Amount}
      ...
      <div className="donate-btn row gx-0 justify-content-center">
        <div className="col option-container">
          <button type="button" className="btn btn-donate btn-rounded btn-block btn-primary" onClick={() => nextStep()}>
            {locale[window.language].checkout}
          </button>
        </div>
      </div>
      <div className="donate-input row gx-0 justify-content-center">
  ...
  const clickOK = (event) => {
    ...
    grecaptcha.ready(() => {
      ...
      grecaptcha.execute('6LdxET4dAAAAAE4ZyQHdePjS0kCAzRdDPPXCJexw', { action: 'submit' }).then(async (token) => {
        ...
        fetch(`/wp-json/proterrasancta-api/v1/donor-newsletter`, {
          method: 'POST',
          mode: 'cors',
          cache: 'no-cache',
          headers: {
...
const CheckoutForm = ({
  ...
  const handleSubmit = async (event) => {
    ...
    if (result.error) {
      ...
      fetch(`/wp-json/proterrasancta-api/v1/checkout`, {
        method: 'POST',
        mode: 'cors',
        cache: 'no-cache',
        headers: {
    ...
    <>
      <div className="donate-notmenu row gx-0">
        ...
        {formType !== 'recurring' && (
          <>
            <div className="col-6">
              <span className={`text-menu selected`}>{locale[window.language].contributo}</span>
            </div>
          </>
        )}
      </div>
