...
class bootstrap_5_walker_nav_menu_sidebar extends Walker_Nav_menu // phpcs:ignore
{
  function start_lvl(&$output, $depth = 0, $args = []) // phpcs:ignore
  {
    $indent  = str_repeat("\t", $depth);
    $submenu = ($depth > 0) ? ' sub-menu' : '';
    $output .= "\n$indent<ul class=\"sidenav-collapse\">\n";
  }
  ...
  function start_el(&$output, $item, $depth = 0, $args = [], $id = 0) // phpcs:ignore
  {
    ...
    $attributes .= ($args->walker->has_children)
      ? ' class="sidenav-link"' // phpcs:ignore
      : ' class="sidenav-link"'; // phpcs:ignore

    // phpcs:disable
    $item_output = $args->before;
    $item_output .=
      $depth > 0 ? '<a class="sidenav-link"' . $attributes . '>' : '<a' . $attributes . '>';
    $item_output .=
